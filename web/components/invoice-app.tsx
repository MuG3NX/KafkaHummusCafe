...
function draftFromRow(row: InvoiceDraftRow | null | undefined): DraftForm {
...
function statusLabel(status: InvoiceRecord["status"]): string {
  ...
  if (status === "abandoned") return "Abandoned — start a new upload";
...
export function InvoiceApp({ onOpenRevenue, onOpenShifts, onOpenCosts }: { onOpenRevenue: () => void; onOpenShifts: () => void; onOpenCosts?: () => void }) {
  ...
  const [persistedDraft, setPersistedDraft] = useState<InvoiceDraftRow | null>(null);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");

  const invoiceSelect = "id, location_id, storage_path, original_filename, original_mime_type, original_size_bytes, status, approved_draft_version, version, created_at, updated_at";

  function draftFields(form: DraftForm): Omit<DraftForm, "reason"> {
    const { reason: _reason, ...fields } = form;
    return fields;
  }

  function formIsDirty(): boolean {
    return persistedDraft !== null && JSON.stringify(draftFields(draft)) !== JSON.stringify(draftFields(draftFromRow(persistedDraft)));
  }

  async function fetchInvoiceDetails(invoiceId: string) {
    if (!supabase) return null;
    const invoiceResult = await supabase.from("invoice_records").select(invoiceSelect).eq("id", invoiceId).single();
    if (invoiceResult.error || !invoiceResult.data) throw invoiceResult.error ?? new Error("Invoice not found");
    const draftResult = await supabase.from("invoice_extraction_drafts").select("*").eq("invoice_id", invoiceId).order("version", { ascending: false }).limit(1).maybeSingle();
    if (draftResult.error) throw draftResult.error;
    const invoice = invoiceResult.data as InvoiceRecord;
    const latest = (draftResult.data ?? null) as InvoiceDraftRow | null;
    let signedUrl = "";
    ...
    return { invoice, draft: latest, signedUrl };
  }
  ...
  async function refreshInvoice(invoiceId: string) {
    ...
    setPersistedDraft(details.draft);
    setDraft(draftFromRow(details.draft));
    setSelectedUrl(details.signedUrl);
    setInvoices((current) => current.map((invoice) => invoice.id === invoiceId ? details.invoice : invoice));
    return details;
  }
  ...
  async function loadWorkspace(options: { clearError?: boolean } = {}) {
    ...
    const invoiceResult = await supabase.from("invoice_records").select(invoiceSelect).eq("location_id", currentLocation.id).order("created_at", { ascending: false });
  ...
  async function openInvoice(invoice: InvoiceRecord) {
    setError("");
    try { await refreshInvoice(invoice.id); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "Invoice could not be loaded."); }
  }
  ...
  async function abandonUpload() {
    ...
    setBusy(true); setError("");
    try {
      const result = await supabase.rpc("abandon_invoice_upload", { p_invoice_id: selected.id, p_reason: "Upload abandoned by user." });
      if (result.error) throw result.error;
      await refreshInvoice(selected.id);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "The upload could not be abandoned."); }
    setBusy(false);
  }
  ...
  async function approveInvoice() {
    if (!supabase || !selected || !owner || !persistedDraft || !canApproveInvoice(selected.status, persistedDraft.version, selected.version, formIsDirty(), busy)) return;
    setBusy(true); setError("");
    const result = await supabase.rpc("approve_invoice", { p_invoice_id: selected.id, p_draft_version: persistedDraft.version, p_invoice_version: selected.version, p_reason: "Human review approved" });
    if (result.error) setError(result.error.message); else await refreshInvoice(selected.id);
    setBusy(false);
  }
  ...
  return <main className="app-shell">
    ...
    {selected?.status === "uploading" && <section className="card"><div className="kicker">UPLOAD RECOVERY</div><h2>{selected.original_filename}</h2><p className="sub">Status: Uploading · version {selected.version}</p><p className="fine">The record is not reviewable until the exact private original is complete.</p>{selectedUrl && <a className="btn secondary" href={selectedUrl} target="_blank" rel="noreferrer">Open private original</a>}<button className="btn" disabled={busy} onClick={() => void completeUpload()}>Complete upload</button><button className="btn secondary" disabled={busy} onClick={() => void abandonUpload()}>Abandon incomplete upload</button></section>}
    {selected?.status === "abandoned" && <section className="card"><div className="kicker">UPLOAD ABANDONED</div><h2>{selected.original_filename}</h2><p className="sub">Status: {statusLabel(selected.status)} · version {selected.version}</p><p className="fine">This record is terminal and cannot be reviewed or approved. Start a new upload when you are ready.</p>{selectedUrl && <a className="btn secondary" href={selectedUrl} target="_blank" rel="noreferrer">Open private original</a>}</section>}
    {selected && selected.status !== "uploading" && selected.status !== "abandoned" && <section className="card"><div className="kicker">HUMAN REVIEW</div><h2>{selected.original_filename}</h2><p className="sub">Status: {statusLabel(selected.status)} · version {selected.version}</p>{selectedUrl && <a className="btn secondary" href={selectedUrl} target="_blank" rel="noreferrer">Open private original</a>}<p className="fine">No OCR provider is connected. Enter or correct the draft fields below; approval is owner-only.</p>
      ...
      {owner && selected.status === "needs_review" && <><button className="btn" disabled={!approvalEnabled} onClick={() => void approveInvoice()}>Approve for future reporting</button>{formIsDirty() && <p className="fine">Save your changes before approval.</p>}<button className="btn secondary" disabled={busy || !draft.reason.trim()} onClick={() => void rejectInvoice()}>Reject with reason</button></>}
    </section>}
  </main>;
}
